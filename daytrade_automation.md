...
### フェーズ1：環境構築（1週間目）
- [ ] 三菱UFJ eスマート証券の口座開設完了後：kabuステーションのインストール・API有効化
- [ ] Claude Codeのインストール・ログイン
- [ ] Pythonのセットアップ（Claude Codeに任せる）
- [x] J-Quants APIのアカウント登録（完了）
- [ ] J-Quants APIのLightプラン登録・APIキー取得
- [ ] TDnet MCPの設定

### フェーズ2：スクリーニング自動化（1〜2週間目）
- [ ] J-Quants APIで全銘柄データ取得スクリプト作成
- [ ] フィルタ条件の実装（上昇率・売買代金・予算内）
- [ ] 候補銘柄リストの出力
- [ ] 動作確認・精度検証

### フェーズ3：数値ベースチャート判定（2〜3週間目）
- [ ] kabuステーションAPIでリアルタイムOHLCV取得スクリプト作成
- [ ] VWAP・移動平均をPythonで計算する関数実装
- [ ] エントリー判定ロジックの実装（パターンA・B）
- [ ] 5分ごとの定期実行設定（Windowsタスクスケジューラ）

### フェーズ4：判定精度の検証（3〜4週間目）
- [ ] 判定ロジックのバックテスト（過去データで検証）
- [ ] パラメータチューニング（VWAPタッチ幅・上昇率閾値など）
- [ ] シグナル精度が十分でなければフェーズ4を繰り返す
