...
def main():
    ...
    # STEP 2: 株価データを期間指定で一括取得
    logger.info("\n" + "=" * 60)
    logger.info("STEP 2: 株価データ一括取得（60営業日分）")
    logger.info("=" * 60)
    ...
    logger.info("全銘柄×60営業日分を一括取得中... (数分かかる場合があります)")

    try:
        df_all = jquants.client.get_eq_bars_daily_range(
            start_dt=start_date,
            end_dt=end_date
        )
        logger.success(f"株価データ取得完了: {len(df_all):,}行")
