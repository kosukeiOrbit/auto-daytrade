...
【設計上の制約】
銘柄ソースとして前日スクリーニング済みの candidates_*.csv を使用している。
本番のパターンBはkabuステーションの売買高急増ランキングAPI（当日リアルタイム）
から銘柄を検出するため、candidates外の銘柄も対象になる。

この制約によるバックテスト精度への影響：
- 過小評価: 本番ではcandidates外の銘柄もエントリー対象になり機会が増える
- 過大評価: バックテストはcandidates全銘柄（数十〜100銘柄）を走査するが、
  本番のランキングAPIは上位10位のみ。走査対象が広い分有利なエントリーを
  見つけやすい

全銘柄の分足走査は1ヶ月約1000万行で非現実的なため、この設計を採用している。
"""
import os
import glob
import pandas as pd
