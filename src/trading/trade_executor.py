...
2026-04-23 パターンAを仮想モード（ドライラン）に切り替え。PATTERN_A_REAL_ENTRY=Falseで注文なし情報収集モード
           ...
           仮想モードの銘柄数制限なし（全候補を記録してサンプル数最大化）
...
2026-04-21 逆指値API注文を復活（エントリー後に逆指値発注→失敗時はmonitor_positions監視型にフォールバック）
           3秒ポーリングの遅延による-1%設定なのに-2%で損切りされる問題の解消
2026-04-20 決済時の損益計算を判断時の現在値→実約定価格（get_orders exec_price）に修正
           （成行売りのスリッページが反映されず実損益と乖離する問題を修正）
2026-04-17 ①利確・損切り価格をint()切り捨て→round()四捨五入+最低1円差保証
           （34円銘柄で利確=エントリー価格になるバグ修正）
           ②パターンA発注前とcalculate_position_sizeに200円未満除外を追加
           （2134北浜CP34円のエントリー事例を受けて対応）
2026-04-16 分割約定時のforce_exit偽レコード防止（entry_price fallback to pos['price']）
"""
import os
import time
import pandas as pd
...
class TradeExecutor:
    ...
    def __init__(self, budget=800000, max_daily_loss_rate=0.03, max_consecutive_losses=3):
        ...
        self.max_entry_amount = 500_000  # 1銘柄あたりの投資上限（50万円）

        # 利確・損切り設定（%）
        self.take_profit_pct_a = 2.0   # パターンA利確
    ...
    def calculate_position_size(self, current_price):
        ...
        if current_price < 200:
            logger.warning(f"株価{current_price}円 < 200円のため発注スキップ")
            return 0
    ...
    def entry_with_stop_and_target(self, symbol, exchange=9, entry_pattern='B'):
        ...
        try:
            ...
            # 最低株価チェック（200円未満は板薄・低位株リスク）
            if current_price < 200:
                logger.info(f"{symbol}: 株価{current_price}円 < 200円のためスキップ（低位株除外）")
            ...
            try:
                ...
                if stop_result.get('result_code') == 0:
                    ...
                    logger.info(f"{symbol}: 逆指値注文成功 損切={stop_price}円 order_id={stop_order_id}")
                else:
                    logger.warning(f"{symbol}: 逆指値注文失敗（result_code={stop_result.get('result_code')}）→ 監視型にフォールバック")
            except Exception as e:
    ...
    def execute_daily_trading(self):
        ...
        for idx, row in candidates_df.iterrows():
            ...
            try:
                ...
                # 気配値GAPチェック（寄り付き前のGD・過大ギャップアップを除外）
                ...
                if bid_price > 0 and ask_price > 0:
                    ...
                    if base_close > 0:
                        ...
                        if pre_gap_pct > self.max_gap_pct:
                            logger.info(f"{symbol}: 気配GAP過大（{pre_gap_pct:+.1f}% > {self.max_gap_pct}%）→ 発注スキップ")
                            self.entry_blacklist.add(symbol)
                            continue
        ...
        for symbol, entry_order_id, qty, current_price, row in pending_orders:
            ...
            target_price = max(
                int(round(actual_entry_price * (1 + self.take_profit_pct_a / 100))),
                int(actual_entry_price) + 1
            )
            ...
            logger.success(f"{symbol}: エントリー完了 {qty}株@{actual_entry_price}円 損切={stop_price} 利確={target_price}（逆指値{'成功' if stop_order_id else '失敗→監視型'}）")
            entry_count += 1
    ...
    def execute_daily_trading_dry_run(self):
        ...
        注意：仮想モードのため銘柄数制限なし（全候補を記録してサンプル数を最大化）
        """
        logger.info("=" * 60)
    ...
    def _wait_for_exit_fill(self, order_id, symbol, fallback_price, timeout=5):
        """決済注文の実約定価格を取得（成行売りの実際のfill価格）"""
        time.sleep(2)
        try:
            orders = self.kabu_client.get_orders(symbol=symbol)
            for order in orders:
                if order.get('order_id') == order_id and order.get('exec_price'):
                    actual = order['exec_price']
                    if actual != fallback_price:
                        logger.info(f"{symbol}: 実約定価格={actual}円（判断時価格={fallback_price}円）")
                    return actual
        except Exception as e:
            logger.warning(f"{symbol}: 約定価格取得失敗、判断時価格を使用: {e}")
    ...
    def force_exit_all_positions_eod(self):
        ...
        try:
            ...
            # 完了通知
            self.notifier.send_message(f"大引け前強制決済完了: {len(aggregated)}銘柄")
    ...
    def generate_daily_report(self, opening_wallet=None, closing_wallet=None, opening_cash=None, closing_cash=None, take_profit_pct=None, stop_loss_pct=None):
        """
        ...
        report = {
            ...
            'take_profit_pct': take_profit_pct if take_profit_pct is not None else self.take_profit_pct_a,
    ...
    def monitor_positions(self):
        ...
        try:
            ...
            for pos in positions:
                ...
                if qty <= 0:
                    ...
                    if symbol in self.active_positions:
                        ...
                        del self.active_positions[symbol]
                        self.entry_blacklist.add(symbol)
                        logger.info(f"{symbol}: 逆指値決済後ブラックリスト追加（当日中再エントリー禁止）")
                    continue
            ...
            for symbol, pos_info in list(self.active_positions.items()):
                ...
                # 逆指値注文が有効な場合はAPI側に任せる（監視型はフォールバックのみ）
                stop_order_id = pos_info.get('stop_order_id')
                if stop_order_id:
                    try:
                        orders = self.kabu_client.get_orders(symbol=symbol)
                        stop_order = next(
                            (o for o in orders if o['order_id'] == stop_order_id),
                            None
                        )
                        if stop_order and stop_order.get('state') == 5:
                            entry_price = pos_info.get('entry_price', 0)
    ...
    def scan_pattern_b_candidates(self):
        ...
        try:
            ...
            for item in ranking:
                ...
                # 予算上限フィルター（1単元100株が1銘柄上限内か）
                if current_price * 100 > self.max_entry_amount:
                    logger.info(f"パターンB除外（予算超過）: {symbol} 現在値{current_price}円 必要額{current_price*100:,.0f}円 上限{self.max_entry_amount:,.0f}円")
                    continue
